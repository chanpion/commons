...
import com.chenpp.common.bigdata.security.KerberosUtil;
...
public class HBaseUtil {
    ...
    public static User getAuthenticatedUser(String krb5Path, String keytabPath, String principal) {
        ...
        try {
            KerberosUtil.checkKerberos(krb5Path, keytabPath, principal);
            loginedUser = User.create(UserGroupInformation.getLoginUser());
        } catch (IOException e) {
            logger.error(String.format("kerberos auth error: principal=%s, keytabPath=%s, krb5Path=%s", principal, keytabPath, krb5Path), e);
        }
        return loginedUser;
    }
        ...
        if (connection == null || connection.isClosed()) {
            synchronized (HBASE_CONN_POOL_MAP) {
                ...
                if (connection == null || connection.isClosed()) {
                    ...
                    if (hbaseConf.getKerberosEnable()) {
                        User user = getAuthenticatedUser(hbaseConf.getKrb5Path(), hbaseConf.getKeytabPath(), hbaseConf.getPrincipal());
    ...
    public static List<String> listNamespaces(HBaseConf hbaseConf) throws IOException {
        Connection connection = connect(hbaseConf);
        Admin admin = connection.getAdmin();
        NamespaceDescriptor[] namespaces = admin.listNamespaceDescriptors();
        return Arrays.stream(namespaces).map(NamespaceDescriptor::getName).collect(Collectors.toList());
    }
    ...
    public static HbaseRow getOneRow(Connection connection, String tableName, String rowKey, String... columnFamily) throws IOException {
        HbaseRow row;
